    ...
    <div class="relative z-[100] selector">
      ...
      <transition name="fade">
        <div
          ...
          class="language-window absolute z-[10] top-[25px] w-[107%] border border-solid border-[#ffffff33] rounded-8 translate-y-[8px]"
        >
          <ul
            @click="showOptions = !showOptions"
            class="w-full overflow-hidden text-left list-none duration-500 ease-in-out bg-transition options-wrapper"
          >
            <li
              ...
              :key="index"
              @click="select(item)"
              class="option flex items-center gap-[7.5px] text-[13px] font-bold leading-[16px] group transition duration-300 ease-in-out py-[10px] px-[12px]"
            >
              <div
              ...
              >
                <u-icons
                  name="globe_white"
                  class="flex w-full items-center h-full transition duration-500 ease-in-out"
                  :class="{ current: item === selectedOption }"
                />
              </div>
              <span class="group-hover:text-white">{{ item.title }}</span>
            </li>
          </ul>
        </div>
      </transition>
